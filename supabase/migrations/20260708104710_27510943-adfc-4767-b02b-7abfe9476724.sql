...
-- Recreate has_role in public as SECURITY INVOKER (no elevated privileges).
-- Callers can already read their own row in public.user_roles via the
-- "roles self read" policy, so admin checks continue to work.
...
-- Point every policy back at public.has_role, then drop the private copy.
...
drop policy if exists "health admin insert" on public.health_logs;
